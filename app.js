...
function buildMarket(item,all,query,settings){
  ...
  const priceReason=strictIdentity?(strictReady?'厳密一致した同等品2件以上を使って売価を算定':assistedReady?'厳密一致1件以上＋高一致を含む3件以上・2サイト以上で参考売価を算定':'厳密一致・高一致の比較根拠が不足しているため売価を算定できません'):(priceReady?'高一致を含むカテゴリ相場4件以上・2サイト以上を使って参考売価を算定':'高一致を含むカテゴリ相場4件以上・2サイト以上の比較根拠が不足しています');
...
function opportunity(item,m,settings){
  ...
  if(risk.level===2){verdict='PASS';reason.push((risk.reasons||[]).join('・')||'故障・欠品等の強い注意')}
    ...
    const buyReady=m.buyEvidenceReady&&m.aCount>=2&&m.confidenceScore>=65&&std.profit>=settings.minProfit&&std.roi>=settings.minRoi&&low.profit>=Math.max(500,settings.minProfit*.20);
    if(buyReady){verdict='BUY';reason.push('厳密一致2件以上・利益・投資利益率・下振れ耐性が基準到達')}
    else if(std.profit>=settings.watchProfit&&std.roi>=settings.watchRoi&&m.confidenceScore>=50){verdict='WATCH';reason.push(m.assistedReady?'近似同等品では利益余地あり。ただし厳密一致2件未満のため要確認':'利益余地あり。ただし仕入候補の安全条件には未到達')}
...
function modeLabel(m){return({strict:'厳密一致',assisted:'近似同等品参考相場','strict-insufficient':'厳密一致不足',category:'カテゴリ参考相場','category-insufficient':'カテゴリ相場不足'})[m]||m}
