...
const SEARCH_JOB='sedori_search_job_v451';
const SEARCH_RESULT='sedori_search_result_v451';
const LOGIN='sedori_login_v45';
const LOGIN_TIME='sedori_login_time_v45';
const LOGIN_JOB='sedori_login_job_v451';
const LOGIN_REPORT='sedori_login_report_v451';
const LEGACY_KEYS=['sedori_job_v4','sedori_result_v4','sedori_login_check_v42','sedori_login_job_v42','sedori_login_job_v43','sedori_login_report_v43','sedori_search_job_v44','sedori_search_result_v44','sedori_login_job_v44','sedori_login_report_v44','sedori_search_job_v45','sedori_search_result_v45','sedori_login_job_v45','sedori_login_report_v45'];
...
function markConnected(){document.documentElement.dataset.sedoriUserscript=VERSION;document.documentElement.dataset.sedoriBridge='dom-gm-v451';lockLegacyHandlers()}
...
const SEL={mercari:'a[href*="/item/"],a[href*="/shops/product/"]',rakuma:'a[href*="/products/"],a[href*="/item/"],a[href*="/shop/"]',yahoo_fleamarket:'a[href*="/item/"]',yahoo_auction:'a[href*="/jp/auction/"]',jmty:'a[href*="/sale-"]'};
function isProductUrl(site,href){const h=String(href||'');return site==='mercari'?/\/item\/|\/shops\/product\//.test(h):site==='rakuma'?/\/products\/|\/item\/|\/shop\//.test(h):site==='yahoo_fleamarket'?/\/item\//.test(h):site==='yahoo_auction'?/\/jp\/auction\//.test(h):site==='jmty'?/\/sale-/.test(h):false}
function rootFor(a){return a.closest('article,li,[data-testid*="item"],[data-testid*="product"],[class*="item"],[class*="Item"],[class*="product"],[class*="Product"],[class*="card"],[class*="Card"],section')||a.parentElement||a}
...
function nodePrice(r,txt){for(const el of [r,...r.querySelectorAll?.('[data-price],[aria-label*="円"],[aria-label*="¥"],[class*="price"],[class*="Price"]')||[]]){const raw=el?.getAttribute?.('data-price')||el?.getAttribute?.('aria-label')||el?.textContent||'';const p=price(raw);if(p)return p}return price(txt)}
function nodeTitle(a,r,txt){const named=r.querySelector?.('h1,h2,h3,[data-testid*="name"],[data-testid*="title"],[class*="name"],[class*="Name"],[class*="title"],[class*="Title"]');return (a.getAttribute('aria-label')||a.getAttribute('title')||r.querySelector?.('img')?.alt||named?.textContent||a.innerText||txt.replace(/[¥￥]\s*[\d,]+.*$/,'')).replace(/\s+/g,' ').trim().slice(0,180)}
function collect(site,f){const map=new Map(),max=Math.max(10,Math.min(80,+f.maxPerSite||50));let anchors=[...document.querySelectorAll(SEL[site]||'a[href]')];if(anchors.length<3)anchors=[...document.querySelectorAll('a[href]')].filter(a=>isProductUrl(site,a.href));for(const a of anchors){if(!isProductUrl(site,a.href))continue;const r=rootFor(a),txt=(r.innerText||a.innerText||'').replace(/\s+/g,' ').trim();if(!txt||txt.length>3500)continue;const p=nodePrice(r,txt),img=r.querySelector?.('img');const title=nodeTitle(a,r,txt),url=(a.href||'').split('?')[0];if(!p||!title||!url)continue;if((f.min&&p<f.min)||(f.max&&p>f.max))continue;if(f.onSale&&/売り切れ|SOLD|取引終了|受付終了|終了しました|落札済/i.test(txt))continue;if(f.excludeAds&&(/広告|スポンサー|おすすめショップ|プロモーション|\bPR\b/i.test(txt)||r.closest?.('[data-testid*="ad"],[class*="advert"],[class*="sponsor"]')))continue;const cond=conditionText(txt);if(f.condition==='new'&&cond!=='new')continue;if(f.condition==='good'&&cond==='used')continue;if(!map.has(url))map.set(url,{source:site,title,price:p,url,image:img?.currentSrc||img?.src||'',condition:cond,rawText:txt.slice(0,800)})}return[...map.values()].slice(0,max)}
async function stableCollect(site,f){let best=[],same=0,last=-1;for(let i=0;i<12;i++){await sleep(i?700:1200);if(i===2||i===5||i===8){try{window.scrollTo(0,Math.min(document.documentElement.scrollHeight,(i+1)*900));await sleep(350)}catch{}}const now=collect(site,f);if(now.length>best.length)best=now;if(now.length===last&&now.length>0)same++;else same=0;last=now.length;if(same>=3||best.length>=Math.min(48,+f.maxPerSite||50))break}try{window.scrollTo(0,0)}catch{}return best}
...
async function beginSearch(f){if(!f.query)return;await clearLegacyJobs();const q=queue(f),job={version:451,filters:f,queue:q,index:0,items:[],siteCounts:{},errors:{},returnUrl:location.href.split('#')[0],started:Date.now()};if(!(await set(SEARCH_JOB,job)))throw new Error('検索ジョブを保存できません');location.assign(q[0][1])}
async function beginLogin(){await clearLegacyJobs();const checkId=`${Date.now()}-${Math.random().toString(36).slice(2,8)}`,job={version:451,index:0,sites:SITE_IDS,returnUrl:location.href.split('#')[0],started:Date.now(),checkId},report={checkId,started:job.started,statuses:{},times:{},done:false,userscriptVersion:VERSION};if(!(await set(LOGIN_JOB,job)))throw new Error('ログイン確認を保存できません');if(!(await set(LOGIN_REPORT,report)))throw new Error('ログイン結果を初期化できません');location.assign(LOGIN_URLS[SITE_IDS[0]])}
...
async function processSearchJob(site){const job=await get(SEARCH_JOB,null);if(!job)return false;if(Date.now()-job.started>10*60*1000){await del(SEARCH_JOB);return false}const expected=job.queue?.[job.index];if(!expected||expected[0]!==site)return false;let items=[],err='';try{const state=await sampleLoginState(site);await saveLogin(site,state);items=await stableCollect(site,job.filters);if(!items.length){const anchors=[...document.querySelectorAll('a[href]')].filter(a=>isProductUrl(site,a.href)).length;err=state==='out'?'要ログイン':state==='unknown'?`ログイン状態不明／商品取得0件（商品リンク${anchors}件）`:`商品取得0件（商品リンク${anchors}件）`}}catch(e){err='取得エラー';console.error(e)}job.items.push(...items);job.siteCounts[site]=items.length;job.errors[site]=err;job.index++;if(job.index<job.queue.length){await set(SEARCH_JOB,job);location.assign(job.queue[job.index][1]);return true}const dedup=[...new Map(job.items.map(x=>[x.url,x])).values()],counts=Object.fromEntries(SITE_IDS.map(id=>[id,dedup.filter(x=>x.source===id).length]));await set(SEARCH_RESULT,{items:dedup,siteCounts:counts,errors:job.errors,started:job.started,finished:Date.now(),userscriptVersion:VERSION});await del(SEARCH_JOB);location.assign((job.returnUrl||'https://m3924saim-eng.github.io/sedori-ai/')+'#search');return true}
