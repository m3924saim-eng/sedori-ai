// ==UserScript==
// @name         せどりAI v4.4 安定版 5サイト検索＋ログイン同期
// @namespace    https://m3924saim-eng.github.io/
// @version      4.4.0
// @description  iPhone/iPad Safari Userscripts向け。postMessage依存をやめ、DOMブリッジ＋GM保存で5サイト検索とログイン確認を安定化。
...
const SEARCH_JOB='sedori_search_job_v44';
const SEARCH_RESULT='sedori_search_result_v44';
const LOGIN='sedori_login_v44';
const LOGIN_TIME='sedori_login_time_v44';
const LOGIN_JOB='sedori_login_job_v44';
const LOGIN_REPORT='sedori_login_report_v44';
const DRAFT='sedori_draft_v44';
...
const SELL_URLS={mercari:'https://jp.mercari.com/sell',rakuma:'https://fril.jp/',yahoo_fleamarket:'https://paypayfleamarket.yahoo.co.jp/sell',yahoo_auction:'https://auctions.yahoo.co.jp/',jmty:'https://jmty.jp/'};
...
function collect(site,f){
  ...
  document.querySelectorAll(SEL[site]||'a').forEach(a=>{
    const r=rootFor(a),txt=(r.innerText||a.innerText||'').replace(/\s+/g,' ').trim();
    ...
    if(!map.has(url))map.set(url,{source:site,title,price:p,url,image:img?.currentSrc||img?.src||'',condition:cond,rawText:txt.slice(0,600)});
  });
...
function loginState(site){
  const body=(document.body?.innerText||'').replace(/\s+/g,' ').slice(0,200000);
  const path=(location.pathname||'').toLowerCase();
  const hrefs=[...document.querySelectorAll('a[href]')].slice(0,3500).map(a=>a.getAttribute('href')||'').join(' ');
  if(/\/(login|signin|sign_in|auth)(?:\/|$)/.test(path)||/ログインしてください|ログインが必要|ログインして続行/.test(body))return'out';
  const rules={
  ...
  if(rules){if(rules.inHref.test(hrefs)||rules.inText.test(body))return'in';if(rules.outHref.test(hrefs)||rules.outText.test(body))return'out'}
  return'unknown';
}
...
async function beginSearch(f){
  if(!f.query)return;
  const q=queue(f),job={version:44,filters:f,queue:q,index:0,items:[],siteCounts:{},errors:{},returnUrl:location.href.split('#')[0],started:Date.now()};
  if(!(await set(SEARCH_JOB,job)))throw new Error('検索ジョブを保存できません');
  location.assign(q[0][1]);
}
async function beginLogin(){
  const checkId=`${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
  const job={version:44,index:0,sites:SITE_IDS,returnUrl:location.href.split('#')[0],started:Date.now(),checkId};
  const report={checkId,started:job.started,statuses:{},times:{},done:false,userscriptVersion:VERSION};
  if(!(await set(LOGIN_JOB,job)))throw new Error('ログイン確認を保存できません');
  if(!(await set(LOGIN_REPORT,report)))throw new Error('ログイン結果を初期化できません');
  location.assign(LOGIN_URLS[SITE_IDS[0]]);
}
async function processCommand(){
  const el=document.getElementById('sedoriBridgeCommand');if(!el||el.dataset.processing)return;
  el.dataset.processing='1';let c;try{c=JSON.parse(el.textContent||'{}')}catch{el.remove();return}
  el.remove();
  try{if(c.type==='search')await beginSearch(c.filters||filtersFromDom());else if(c.type==='login')await beginLogin()}catch(e){console.error(e);bridge('sedoriBridgeError',{message:String(e?.message||e),at:Date.now()})}
}

function loginHtml(id,state,time){const label=state==='in'?'ログイン済み':state==='out'?'要ログイン':state==='unknown'?'判定できず':'未確認';const cls=state==='in'?'status-ok':state==='out'?'status-ng':'';const when=time?`<small style="display:block;color:#667085;font-size:10px;margin-top:2px">確認 ${new Date(time).toLocaleString('ja-JP')}</small>`:'';return `<div class="login-row"><span><b>${N[id]}</b>${when}</span><span class="${cls}">${label}</span></div>`}
...
function markConnected(){document.documentElement.dataset.sedoriUserscript=VERSION;document.documentElement.dataset.sedoriBridge='dom-gm-v44'}

async function appMode(){
  markConnected();
  const old=document.querySelector('a[href^="sedori-ai.user.js"]');if(old){old.href='sedori-ai.user.js';old.textContent=`v${VERSION} を開く`}
  const result=await get(SEARCH_RESULT,null);if(result){await del(SEARCH_RESULT);bridge('sedoriBridgeResult',result)}
  await renderLogin();
  await processCommand();
  new MutationObserver(()=>processCommand()).observe(document.documentElement,{childList:true,subtree:true});
  setInterval(()=>{markConnected();processCommand()},700);
}

async function processLoginJob(site){
  const job=await get(LOGIN_JOB,null);if(!job)return false;
  if(Date.now()-job.started>5*60*1000){await del(LOGIN_JOB);return false}
  if(job.sites?.[job.index]!==site)return false;
  const report=await get(LOGIN_REPORT,{checkId:job.checkId,started:job.started,statuses:{},times:{},done:false});
  const state=await sampleLoginState(site);await saveLogin(site,state);
  report.statuses={...(report.statuses||{}),[site]:state};report.times={...(report.times||{}),[site]:Date.now()};
  job.index++;
  if(job.index<job.sites.length){await set(LOGIN_REPORT,report);await set(LOGIN_JOB,job);location.assign(LOGIN_URLS[job.sites[job.index]]);return true}
  report.done=true;report.finished=Date.now();await set(LOGIN_REPORT,report);await del(LOGIN_JOB);location.assign((job.returnUrl||'https://m3924saim-eng.github.io/sedori-ai/')+'#search');return true;
}
async function processSearchJob(site){
  const job=await get(SEARCH_JOB,null);if(!job)return false;
  if(Date.now()-job.started>10*60*1000){await del(SEARCH_JOB);return false}
  const expected=job.queue?.[job.index];if(!expected||expected[0]!==site)return false;
  let items=[],err='';try{const state=await sampleLoginState(site);await saveLogin(site,state);items=await stableCollect(site,job.filters);if(!items.length)err=state==='out'?'要ログイン':state==='unknown'?'ログイン状態不明／商品取得0件':'商品取得0件'}catch(e){err='取得エラー';console.error(e)}
  job.items.push(...items);job.siteCounts[site]=items.length;job.errors[site]=err;job.index++;
  if(job.index<job.queue.length){await set(SEARCH_JOB,job);location.assign(job.queue[job.index][1]);return true}
  const dedup=[...new Map(job.items.map(x=>[x.url,x])).values()];await set(SEARCH_RESULT,{items:dedup,siteCounts:job.siteCounts,errors:job.errors,started:job.started,finished:Date.now(),userscriptVersion:VERSION});await del(SEARCH_JOB);location.assign((job.returnUrl||'https://m3924saim-eng.github.io/sedori-ai/')+'#search');return true;
}

function fields(){return{title:['input[name*="title"]','input[placeholder*="タイトル"]','input[placeholder*="商品名"]'],desc:['textarea[name*="description"]','textarea[placeholder*="説明"]','textarea'],price:['input[name*="price"]','input[placeholder*="価格"]','input[inputmode="numeric"]']}}
...
async function applyDraft(site){const d=await get(DRAFT,null);if(!d||d.targetSite!==site||!d.draft)return false;const f=fields(),ok=[input(find(f.title),d.draft.title),input(find(f.desc),d.draft.description),input(find(f.price),String(d.draft.price))].some(Boolean);if(ok){const n=document.createElement('div');n.textContent='せどりAIの下書きを入力しました。内容を確認してから保存・出品してください。';n.style.cssText='position:fixed;z-index:99999;left:10px;right:10px;bottom:10px;padding:11px;background:#111827;color:#fff;border-radius:10px;font-size:13px';document.body.append(n);setTimeout(()=>n.remove(),6000);await set(DRAFT,{...d,appliedAt:Date.now()});return true}return false}
...
let tries=0;const timer=setInterval(async()=>{tries++;if(await applyDraft(site)||tries>20)clearInterval(timer)},1000);
})();